...
- 已完成文档校准，与当前 Mindmap 子项目实现一致。
- 该子项目与本轮可观测性主线改造无直接耦合，现有说明保持有效。
...
1. **安装 Python 依赖（推荐使用包安装）**:
```bash
# 方式1：安装为可编辑包（推荐，支持热重载）
pip install -e .
...
# 方式2：使用 uvicorn 直接运行
uvicorn mindmap.main:app --reload

# 方式3：作为模块运行
python -m mindmap.main
```
...
python -m PyInstaller --onefile \
  --add-data "config.ini;." \
  --add-data "htmljs;htmljs" \
  --add-data "static;static" \
  --hidden-import mindmap \
  --hidden-import mindmap.config \
  --hidden-import mindmap.modules \
  --hidden-import mindmap.modules.mindmap_service \
  --hidden-import mindmap.modules.file_service \
  main.py
```
...
项目采用清晰的模块化架构，符合 Python 标准包结构（src 布局），功能分离，易于维护：
...
#### 2. src/mindmap/main.py - FastAPI 应用
- FastAPI 应用初始化
- 动态静态文件挂载
- 路由注册和管理
- 统一的 API 文档生成
...
#### 4. src/mindmap/modules/mindmap_service.py - 思维导图服务
...
#### 5. src/mindmap/modules/file_service.py - 文件管理服务
...
├── pyproject.toml         # 项目配置文件（支持 src 布局）
...
1. **访问思维导图页面** - 使用 `/upload-local` 接口生成的页面（需要配置 `enable_svg_download_button = true`）
2. **点击下载按钮** - 页面右上角的"下载SVG"按钮
3. **等待生成完成** - 系统自动禁止页面操作，显示进度
4. **自动下载文件** - 生成完成后自动下载SVG文件
5. **恢复页面操作** - 自动恢复所有页面功能
...
# 运行特定测试文件
pytest tests/test_mindmap_service.py
...
6. **标准包结构**: 项目采用标准的 src 布局，符合 Python 包结构最佳实践
7. **模块化架构**: 功能已分离到不同模块，便于维护和扩展
8. **包安装**: 推荐使用 `pip install -e .` 安装为可编辑包，支持热重载和模块导入
9. **流式处理**: 文件上传使用流式处理，避免大文件内存问题
10. **热重载**: 开发模式支持代码热重载
11. **静态文件暴露**: JS 文件通过 HTTP 端口对外暴露，支持跨域访问
12. **打包部署**: 支持 PyInstaller 单文件打包，便于部署
13. **离线部署**: 需要是 npm install markmap --prefix ./node_modules_global 将依赖存储到当前的文件夹以备迁移
14. **SVG下载功能**: 使用 `/upload-local` 接口生成的思维导图支持SVG下载（需要配置 `enable_svg_download_button = true`）
15. **页面操作禁止**: SVG生成过程中会自动禁止所有页面操作
16. **图标化界面**: 下载按钮使用图标+文字组合，界面更美观
17. **SVG格式优势**: SVG矢量图支持任意缩放，永不失真，适合打印和展示
18. **浏览器兼容性**: SVG下载功能需要现代浏览器支持，建议使用Chrome、Firefox、Edge等

## 开发指南

### 扩展功能

由于采用模块化架构和标准包结构，可以轻松扩展功能：

1. **添加新的文件处理功能**：在 `src/mindmap/modules/file_service.py` 中扩展 `FileService` 类
2. **添加新的思维导图功能**：在 `src/mindmap/modules/mindmap_service.py` 中扩展 `MindmapService` 类
3. **添加新的路由**：在 `src/mindmap/main.py` 中注册新的 API 端点
4. **修改配置**：在 `src/mindmap/config.py` 中添加新的配置项
5. **添加新的静态文件类型**：在 `src/mindmap/config.py` 中的 `STATIC_FILES_CONFIG` 中添加配置
6. **扩展SVG下载功能**：在 `src/mindmap/modules/mindmap_service.py` 中修改SVG生成逻辑
7. **自定义页面操作禁止**：在 `disablePageOperations()` 函数中添加更多操作限制
8. **优化用户界面**：在 `addSaveButton()` 函数中自定义按钮样式和图标
9. **添加新的模块**：在 `src/mindmap/` 下创建新的模块目录
10. **运行测试**：使用 `pytest tests/` 运行测试套件
