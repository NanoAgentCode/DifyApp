...
- 已完成前端文档全量校准，与当前可观测性展示实现保持一致。
- 已对齐日志详情步骤时间线、`PARTIAL` 状态提示与响应内容 Markdown 渲染能力。
- 已对齐日志列表删除后即时移除与延迟刷新的一致性策略。
...
- **日志监控视图增强（Observability）**：
  - 详情页新增步骤时间线（step 状态、耗时、输入/输出/错误摘要）
  - 展示异步一致性状态（如 `PARTIAL`）并提供告警提示
  - 列表页支持“部分成功”状态展示
- **响应内容渲染优化**：
  - 日志监控详情中的响应内容支持 Markdown 渲染
  - 渲染链路采用轻量模式，兼顾可读性与性能
- **删除交互优化**：
  - 删除日志成功后先本地移除，再延迟刷新列表，减少 ES 近实时读取导致的“短暂残留”感知
- **列表性能优化**：
  - 可观测性列表状态计算改为预处理字段，避免渲染期重复 JSON 解析
  - 知识库管理页面将 `v-for + filter` 热路径改为 computed 复用
