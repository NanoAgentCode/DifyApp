...
- 已完成项目级文档全量校准，覆盖根文档、前后端文档与 `backend/doc` 设计文档。
- 已对齐可观测性升级（`ops.trace` 模块化、步骤追踪、异步 `PARTIAL` 一致性、前端日志监控增强）。
- 已对齐链路一致性规则（同请求 `traceId` 继承与降级兜底）及会话 ID 统一解析规范。
...
- **认证与稳定性**：JWT 拦截器统一鉴权、系统配置 API（`/api/system-config`）支持前端按 key/分组拉取、401 时前端统一提示与单次重定向；业务与缓存一致性保障（AI 应用删除时清除 apikey 缓存，知识库文档/向量配置变更时 RAG 与 VectorStore 缓存失效，向量策略内部使用线程安全结构并正确释放 ES 客户端）

### 最新功能更新（2026-03）

- **可观测性能力升级**：
  - 新增 `ops.trace` 模块，支持主链路追踪 + 步骤级追踪（`metaData.steps`）
  - 引入异步写入与重试机制，支持 `metaData.asyncState=PARTIAL` 一致性标记
  - 管理端新增步骤查询接口：`GET /api/observability/traces/{id}/steps`
- **追踪一致性优化**：
  - 同一请求允许多条记录（多个 `spanId`），但统一复用 `traceId`
  - 引入“业务主 trace 优先 + AOP 追踪降级兜底 + 去重门控”机制
- **日志监控前端增强**：
  - 详情页支持步骤时间线、异步一致性状态提示
  - 响应内容支持 Markdown 渲染
  - 删除日志后前端先本地移除再延迟刷新，改善 ES 近实时一致性体验
- **代码规范与可维护性优化**：
  - 会话 ID 解析统一通过 `ConversationIdUtil.parseConversationId(...)`
  - 多处高频 `DateTimeFormatter` 复用，减少热路径对象创建

详见后端文档：`backend/doc/16.可观测性功能与使用指南.md`。
