...
- **向量数据库管理**：动态管理向量数据库配置（Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector），支持多实例配置、默认设置、连接测试
- **对话历史管理**：完整的对话会话管理，支持会话记录、消息历史、继续对话、开启新对话
- **文件存储**：基于 MinIO 的对象存储
- **向量存储**：支持 Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector 等多种向量存储方式，可在知识库级别选择
...
- **密码加密**: BCrypt
- **对象存储**: MinIO 8.5.2
- **向量数据库**: Qdrant 1.7.0（可选）、Milvus（可选）、Chroma（可选）、Weaviate（可选）、Elasticsearch（可选）、PgVector（可选）
...
- **Weaviate** (图数据库，语义搜索能力强)
- **Elasticsearch** (搜索引擎，支持全文检索+向量搜索)
- **PgVector** (PostgreSQL 扩展，利用 PostgreSQL 的 ACID 特性)
...
#### 配置 PgVector（可选，用于向量存储）

PgVector 是 PostgreSQL 的向量扩展，如果使用 PostgreSQL 作为主数据库，可以直接使用 PgVector 作为向量存储。
...
1. 在向量库管理界面中添加 PgVector 配置：
   - 类型：选择 "PgVector"
   - 连接地址：JDBC URL 格式，例如 `jdbc:postgresql://localhost:5432/vectordb`
   - 用户名/密码：PostgreSQL 数据库的用户名和密码
   - 超时时间：连接超时时间（毫秒），默认 30000

2. **连接地址格式**：
   - 标准格式：`jdbc:postgresql://host:port/database`
   - 示例：`jdbc:postgresql://localhost:5432/vectordb`
   - 支持自动转换：`postgresql://host:port/db` 或 `host:port/db`

3. **索引说明**：
   - **≤ 2000 维**：自动使用 IVFFlat 索引（适合中小维度）
   - **> 2000 维**：自动使用 HNSW 索引（pgvector 0.5.0+，适合高维）
   - 如果 HNSW 不支持，将使用顺序扫描（功能正常，但查询可能较慢）

**优势：**
- ✅ **事务支持**：利用 PostgreSQL 的 ACID 特性
- ✅ **成熟稳定**：基于 PostgreSQL 的成熟生态
- ✅ **易于管理**：使用熟悉的 SQL 工具管理
- ✅ **数据一致性**：与其他业务数据在同一数据库中

> **注意**：如果使用 Docker 启动的 PostgreSQL，确保使用支持 pgvector 的镜像（如 `ankane/pgvector:latest`）
...
编辑 `frontend/src/config/api.js` 文件，修改 `defaultBaseURL` 的值：
...
完整的打包步骤、前置要求、常见问题等，请参考：[前端打包指南](frontend/BUILD.md)

### 8. 访问应用

- **后端 API**: http://localhost:8081
- **Swagger API 文档**: http://localhost:8081/swagger-ui/index.html
- **前端应用**: 
  - 开发环境：http://localhost:3000（需要先启动前端开发服务器，见步骤7）
  - 生产环境：需要将前端构建产物集成到后端静态资源中，然后访问 http://localhost:8081

> **API 文档**: 详细的 API 文档请访问 Swagger UI：http://localhost:8081/swagger-ui/index.html
...
- ✅ **知识库管理**
  ...
  - 文档自动解析（Apache Tika）
  - 文档分块和向量化
  - 向量存储：支持 Qdrant、FAISS、Milvus、Chroma、Weaviate、Elasticsearch、PgVector 七种存储方式，可在知识库级别选择
    - **Qdrant**：分布式向量数据库，适合生产环境，需要独立服务
    - **FAISS**：本地文件存储，无需额外服务，适合开发测试环境
    - **Milvus**：分布式向量数据库，适合生产环境，需要独立服务，使用 gRPC 协议
    - **Chroma**：轻量级向量数据库，易于部署，支持 HTTP REST API
    - **Weaviate**：图数据库，语义搜索能力强，支持 GraphQL 和 REST API
    - **Elasticsearch**：企业级搜索引擎，支持向量搜索、全文检索和混合搜索
    - **PgVector**：PostgreSQL 扩展，利用 PostgreSQL 的 ACID 特性，适合需要事务支持的场景
  - 支持为知识库选择向量化模型
...
- ✅ **向量数据库管理**
  - 动态管理向量数据库配置（Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector）
  - 支持多实例配置
  - 支持设置默认向量数据库
  - 支持启用/禁用向量数据库
  - 支持连接测试功能
...
- ✅ **管理端界面**
  ...
  - 向量数据库管理（Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector 的配置、默认设置、状态管理、连接测试）
...
- ✅ **UI/UX 增强**
  ...
  - **响应式设计**：完整适配小屏幕客户端（1024×768及以下）
    - 侧边栏自动折叠：窗口宽度小于1024px时自动收起，节省屏幕空间
    - 知识库问答页面自适应：小屏幕时左右分栏自动改为上下布局
    - 消息展示优化：用户消息右对齐，助手消息左对齐，宽度限制在70%以内
    - 动画效果：消息从左右两侧滑入，提升交互体验
    - 全局样式优化：表格、卡片、对话框等组件在小屏幕下自动调整字体和间距
...
**向量存储类型选择：**
- 创建知识库时可以选择使用 Qdrant、FAISS、Milvus、Chroma、Weaviate、Elasticsearch、PgVector
- 已有文档的知识库无法修改向量存储类型
- 默认使用 Qdrant（向后兼容）
...
> ⚠️ **重要提示**：系统现已支持通过数据库动态管理向量数据库配置，无需修改配置文件。建议通过管理端界面配置 PgVector。

**通过管理端配置（推荐）：**

1. 登录管理端
2. 进入"向量数据库管理"页面
3. 添加新配置，类型选择 "PgVector"
4. 配置项：
   - **配置名称**：用于标识的配置名称
   - **连接地址**：JDBC URL 格式，例如 `jdbc:postgresql://localhost:5432/vectordb`
   - **用户名/密码**：PostgreSQL 数据库的用户名和密码（在表单中填写）
   - **超时时间**：连接超时时间（毫秒），默认 30000
   - **启用状态**：是否启用该配置
   - **默认配置**：是否设置为默认向量数据库
   - **连接测试**：测试数据库连接和 pgvector 扩展

**PgVector 说明：**

- **前置要求**：
  - PostgreSQL 11+（推荐 PostgreSQL 14+）
  - 已安装 pgvector 扩展：`CREATE EXTENSION IF NOT EXISTS vector;`
...
- **索引策略**：
  - **≤ 2000 维**：自动使用 IVFFlat 索引（适合中小维度，构建快，内存占用小）
  - **> 2000 维**：自动使用 HNSW 索引（pgvector 0.5.0+，适合高维，查询更快但构建较慢）
  - 如果 HNSW 不支持，将使用顺序扫描（功能正常，但查询可能较慢）
...
- **表结构**：
  - 每个知识库会创建一个独立的表：`kb_vectors_{知识库ID}`
  - 表结构包含：`id`、`document_id`、`chunk_index`、`text`、`embedding`、`created_at`
  - 自动创建向量索引和 document_id 索引以提高查询性能

- **注意事项**：
  - 如果表不存在，系统会在首次向量化时自动创建
  - 删除向量时，如果表不存在会优雅处理，不会报错
  - 索引创建失败时会自动回滚，不影响表的正常使用
...
8. 配置合适的文件上传大小限制
9. **向量存储选择**：
   - 生产环境推荐使用 Qdrant、Milvus 或 PgVector（分布式、高性能）
   - 如果需要事务支持，推荐使用 PgVector（利用 PostgreSQL 的 ACID 特性）
   - 开发测试环境可使用 FAISS（无需额外服务）
   - 如果使用 FAISS，确保存储目录有足够的磁盘空间和写入权限
   - 如果使用 Milvus，确保服务正常运行并确保存储目录有足够的磁盘空间和写入权限
   - 如果使用 PgVector，确保 PostgreSQL 已安装 pgvector 扩展
   - 定期备份向量存储目录（FAISS: `data/faiss`，Milvus: 根据服务配置，PgVector: 随 PostgreSQL 数据库备份）
9. **Redis 配置**：
   - 设置 Redis 密码以提高安全性
...
构建产物在 `frontend/dist` 目录。如需集成到后端，需要将构建产物复制到后端的静态资源目录。

**桌面客户端打包（Tauri）：**

项目支持使用 Tauri 打包成桌面客户端（Windows、macOS、Linux）。

详细打包步骤请参考：[前端打包指南](frontend/BUILD.md)
...
打包完成后，安装包位于 `frontend/src-tauri/target/release/bundle/` 目录。
...
- **KNOWLEDGE_BASE**: 知识库表
  ...
  - 包含向量存储类型（vector_store_type），可选值：`qdrant`、`faiss`、`milvus`、`chroma`、`weaviate`、`elasticsearch`、`pgvector`，默认为 `qdrant`
  - 包含向量数据库实例ID（vector_database_id），关联到 VECTOR_DATABASE 表
...
- **VECTOR_DATABASE**: 向量数据库配置表
  ...
  - 支持 Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector 七种类型
  - 支持启用/禁用状态和默认配置设置
  - 支持额外配置（extra_config，JSON格式，用于存储用户名密码等，如 PgVector 和 Elasticsearch）
  - 用于知识库向量存储功能
