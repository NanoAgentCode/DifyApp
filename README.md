...
DifyApp 是一个基于 Spring Boot 和 Vue 3 构建的企业级 AI 应用平台，提供智能对话、知识库管理、AI 应用管理、Text2SQL 等功能。系统采用前后端分离架构，支持多种向量数据库和 LLM 集成，实现了现代化的 RAG（检索增强生成）架构。

### 核心功能

- **用户认证与授权**：JWT 令牌认证，支持用户注册、登录、密码管理
- **智能对话**：支持 Chat 和 Workflow 两种模式，集成多种大语言模型
- **知识库管理**：文档上传、解析、向量化，支持多种向量数据库
- **RAG 问答**：基于知识库的检索增强生成问答
- **AI 应用管理**：创建和管理 AI 应用，支持可见性控制
- **Text2SQL**：自然语言转 SQL 查询
- **系统管理**：数据源管理、模型管理、Prompt 模板管理
- **MCP 协议支持**：浏览器搜索、地理位置、时间服务等
...
- AI 应用创建与管理
- 聊天对话功能（支持 Chat 和 Workflow 两种模式）
- 对话历史管理（会话、消息）
- Dify API 集成
