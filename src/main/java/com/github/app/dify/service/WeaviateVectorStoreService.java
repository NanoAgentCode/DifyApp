...
public class WeaviateVectorStoreService {
    ...
    @Autowired(required = false)
    private com.github.app.dify.repository.KnowledgeBaseRepository knowledgeBaseRepository;
    ...
    private WebClient getWebClient(Long knowledgeBaseId) {
        ...
        com.github.app.dify.domain.VectorDatabase config = getConfigByType("weaviate");
        if (config != null) {
     ...
     * 根据类型获取配置
     */
    private com.github.app.dify.domain.VectorDatabase getConfigByType(String type) {
        if (vectorDatabaseRepository == null) {
            return null;
        }
        try {
            // 先尝试查找默认的启用配置
            Optional<com.github.app.dify.domain.VectorDatabase> defaultConfig = vectorDatabaseRepository.findDefaultEnabledByType(type);
            if (defaultConfig.isPresent()) {
                return defaultConfig.get();
            }
            
            // 如果没有默认配置，尝试查找第一个启用的配置
            List<com.github.app.dify.domain.VectorDatabase> enabledConfigs = vectorDatabaseRepository.findAllEnabledByType(type);
            if (!enabledConfigs.isEmpty()) {
                return enabledConfigs.get(0);
            }
            
            // 如果连启用的配置都没有，尝试查找任何配置（包括未启用的）
            List<com.github.app.dify.domain.VectorDatabase> allConfigs = vectorDatabaseRepository.findByType(type);
