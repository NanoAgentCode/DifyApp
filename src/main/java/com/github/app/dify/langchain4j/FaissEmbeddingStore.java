...
public class FaissEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public static FaissEmbeddingStore forKnowledgeBase(Long knowledgeBaseId, 
                                                        FaissVectorStoreService faissVectorStoreService) {
        FaissEmbeddingStore store = new FaissEmbeddingStore();
        store.faissVectorStoreService = faissVectorStoreService;
    ...
    public String add(Embedding embedding, TextSegment textSegment) {
        ...
        // 确保索引存在
        faissVectorStoreService.ensureIndex(knowledgeBaseId, vector.size());
        ...
        faissVectorStoreService.upsertVectors(knowledgeBaseId, documentId, vectors, texts, chunkIndices);
    ...
    public List<String> addAll(List<Embedding> embeddings, List<TextSegment> textSegments) {
        ...
        for (java.util.Map.Entry<Long, List<BatchItem>> entry : grouped.entrySet()) {
            ...
            // 确保索引存在
            if (!vectors.isEmpty()) {
                faissVectorStoreService.ensureIndex(knowledgeBaseId, vectors.get(0).size());
            }
    ...
    public List<EmbeddingMatch<TextSegment>> findRelevant(Embedding referenceEmbedding, int maxResults, double minScore) {
        ...
        // 检索
        List<FaissVectorStoreService.SearchResult> searchResults = 
                faissVectorStoreService.searchVectors(knowledgeBaseId, queryVector, maxResults);
        
        // 转换为EmbeddingMatch
        List<EmbeddingMatch<TextSegment>> matches = new ArrayList<>();
        for (FaissVectorStoreService.SearchResult result : searchResults) {
    ...
    public void deleteByDocumentId(Long documentId) {
        faissVectorStoreService.deleteDocumentVectors(knowledgeBaseId, documentId);
    }
