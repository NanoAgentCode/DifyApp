...
import com.github.app.dify.knowledgebase.langchain4j.store.BaseEmbeddingStore;
...
public class RagRetrievalServiceImpl implements RagRetrievalService {
    ...
    public List<RetrievalResult> retrieve(Long knowledgeBaseId, String query, Long embeddingModelId, Integer topK) {
        ...
        try {
            ...
            // 检查是否支持混合搜索
            if (embeddingStore instanceof BaseEmbeddingStore baseStore) {
                logger.info("使用混合搜索增强RAG检索 - 知识库ID: {}, 查询: {}", knowledgeBaseId, query);
                EmbeddingSearchResult<TextSegment> searchResult = baseStore.hybridSearch(query, queryEmbedding,
                        effectiveTopK * 3, 0.0);
                ...
                matches = searchResult.matches(); // 先不设置阈值，获取所有结果
            }

            logger.info("RAG检索原始结果 - 知识库ID: {}, 查询: {}, 原始结果数量: {}, 配置topK: {} (知识库配置: {}, 全局配置: {}), 相似度阈值: {}",
                    knowledgeBaseId, query, matches.size(), effectiveTopK, topK, ragConfig.getTopK(),
