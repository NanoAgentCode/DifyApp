...
public interface VectorStoreStrategy {
     ...
     * 是否支持混合搜索（全文检索 + 向量搜索）
     */
    default boolean isHybridSearchSupported() {
        return false;
    }
     ...
     * 
     * @param knowledgeBaseId 知识库ID
     * @param query           查询文本（用于全文检索）
     * @param queryVector     查询向量（用于向量检索）
     * @param topK            返回前K个结果
     * @return 搜索结果列表
     */
    default List<SearchResult> hybridSearch(Long knowledgeBaseId, String query, List<Float> queryVector, int topK) {
