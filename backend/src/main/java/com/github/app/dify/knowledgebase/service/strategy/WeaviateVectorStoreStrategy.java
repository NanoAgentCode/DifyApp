...
public class WeaviateVectorStoreStrategy extends AbstractHttpVectorStoreStrategy implements VectorStoreStrategy {
    ...
    public List<VectorStoreStrategy.SearchResult> hybridSearch(Long knowledgeBaseId, String query,
        ...
        try {
            ...
            // 构建GraphQL混合搜索查询
            // alpha: 1.0 表示纯向量搜索，alpha: 0.0 表示纯关键词搜索
            // 默认使用 0.5 综合两者
            String graphqlQuery = String.format(
                    "{ Get { %s(hybrid: {query: \"%s\", vector: %s, alpha: 0.5}, limit: %d) { _additional { id score } documentId chunkIndex text knowledgeBaseId } } }",
                    className,
                    query.replace("\"", "\\\""),
                    vectorJson,
                    topK);
            ...
            req.put("query", graphqlQuery);

            logger.info("发送Weaviate混合搜索请求 - 知识库ID: {}, 类名: {}, topK: {}, query: {}",
                    knowledgeBaseId, className, topK, query);
            ...
            logger.debug("Weaviate混合搜索完成 - 知识库ID: {}, topK: {}, 结果数量: {}",
                    knowledgeBaseId, topK, results.size());

            return results;

        } catch (Exception e) {
            logger.error("Weaviate混合搜索失败 - 知识库ID: {}, 类名: {}", knowledgeBaseId, className, e);
            // 失败时回退到纯向量搜索
            return searchVectors(knowledgeBaseId, queryVector, topK);
        }
    }
            ...
            Map<String, Object> response = getWebClient(knowledgeBaseId)
                    ...
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                            clientResponse -> {
                                ...
                                return Mono
                                        .error(new BusinessException("检查类存在性失败", ErrorCode.DATABASE_CONNECTION_ERROR));
                            })
                    .bodyToMono(new ParameterizedTypeReference<Map<String, Object>>() {
                    })
                    .timeout(Duration.ofMillis(getTimeout()))
                    ...
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                            clientResponse -> {
                                ...
                                return errorBodyMono.flatMap(errorBody -> {
                                    ...
                                    return Mono.error(
                                            new BusinessException(errorMsg, ErrorCode.DATABASE_CONNECTION_ERROR));
                                });
                            })
                                    ...
                                    clientResponse -> {
                                        ...
                                        return errorBodyMono.flatMap(errorBody -> {
                                            ...
                                            logger.error("向量插入请求失败 - 知识库ID: {}, 文档ID: {}, 类名: {}, HTTP状态: {}, 错误响应: {}",
                                                    knowledgeBaseId, documentId, className, clientResponse.statusCode(),
                                                    errorBody);
                                ...
                                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                                        clientResponse -> {
                                            ...
                                            return errorBodyMono.flatMap(errorBody -> {
                                                ...
                                                logger.error(
                                                        "删除向量请求失败 - 知识库ID: {}, 文档ID: {}, 对象ID: {}, HTTP状态: {}, 错误响应: {}",
                                                        knowledgeBaseId, documentId, objectId,
                                    ...
                                    clientResponse -> {
                                        ...
                                        return errorBodyMono.flatMap(errorBody -> {
                                            ...
                                            logger.error("批量删除向量请求失败 - 知识库ID: {}, 文档ID: {}, HTTP状态: {}, 错误响应: {}",
                                                    knowledgeBaseId, documentId, clientResponse.statusCode(),
                                                    errorBody);
                                            return Mono.error(new BusinessException(errorMsg,
                                                    ErrorCode.DATABASE_CONNECTION_ERROR));
                                        });
                            ...
                            getWebClient(knowledgeBaseId)
                                    ...
                                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                                            clientResponse -> {
                                                ...
                                                return Mono.error(new BusinessException("删除对象失败",
                                                        ErrorCode.DATABASE_CONNECTION_ERROR));
                                            })
                                    .bodyToMono(String.class)
                                    .timeout(Duration.ofMillis(getTimeout()))
