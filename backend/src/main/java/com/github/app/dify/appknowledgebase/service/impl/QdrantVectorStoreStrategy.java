...
import com.github.app.dify.appsystemdata.config.QdrantConfig;
...
public class QdrantVectorStoreStrategy implements VectorStoreStrategy {
        ...
        try {
            ...
            try {
                Map<String, Object> response = getWebClient()
                        ...
                        .timeout(Duration.ofMillis(qdrantConfig.getTimeout()))
                        .block();
    ...
    public void upsertVectors(Long knowledgeBaseId, Long documentId, 
        ...
        try {
            ...
            // 检查集合配置，确定是否使用命名向量
            boolean useNamedVectors = checkIfCollectionUsesNamedVectors(collectionName);
            ...
            // 记录插入前的点数
            long pointsCountBefore = getCollectionPointsCount(collectionName);
            ...
            try {
                ...
                long pointsCountAfter = getCollectionPointsCount(collectionName);
                ...
                if (pointsCountAfter < expectedCount) {
                    ...
                    // 再等待一段时间后重试检查
                    Thread.sleep(1000);
                    pointsCountAfter = getCollectionPointsCount(collectionName);
    ...
    public List<VectorStoreStrategy.SearchResult> searchVectors(Long knowledgeBaseId, List<Float> queryVector, int topK) {
        ...
        // 检查集合是否为空（没有向量点）
        long pointsCount = getCollectionPointsCount(collectionName);
        if (pointsCount == 0) {
