...
public class KnowledgeBaseServiceImpl implements KnowledgeBaseService {
    ...
    public KnowledgeBaseResp createKnowledgeBase(CreateKnowledgeBaseReq req, Long userId, String username, Integer role,
        ...
        if (force == null || !force) {
            ...
            if (!existingKbs.isEmpty()) {
                throw new BusinessException("已存在名称为 \"" + req.getName() + "\" 的知识库，是否继续创建？",
        ...
        if (role == null || role != 1) {
            ...
            logger.debug("管理员创建知识库 - 用户ID: {}, 名称: {}, 是否公开: {}", userId, knowledgeBase.getName(),
    ...
    private void setDefaultVectorDatabase(KnowledgeBase knowledgeBase) {
        ...
        try {
            // 查找所有启用的向量库配置
            List<com.github.app.dify.knowledgebase.domain.VectorDatabase> enabledConfigs = vectorDatabaseRepository
                    .findAllEnabled();

            if (enabledConfigs != null && !enabledConfigs.isEmpty()) {
                // 查找第一个默认的配置（is_default = true）
                Optional<com.github.app.dify.knowledgebase.domain.VectorDatabase> defaultConfig = enabledConfigs
                        .stream()
    ...
    public String generateSummary(Long knowledgeBaseId, Long modelId) {
        ...
        if (systemPrompt.trim().isEmpty()) {
            ...
            if (!fallback.trim().isEmpty()) {
                ...
                systemPrompt = SkillLoader.loadSkill("knowledge_base/document_summary_system_prompt_default");
            }
        }
        ...
        if (!userPromptTemplate.trim().isEmpty()) {
            ...
            userPrompt = SkillLoader
                    .loadSkillWithTemplate("knowledge_base/document_summary_user_prompt_fallback_template", variables);
        }
