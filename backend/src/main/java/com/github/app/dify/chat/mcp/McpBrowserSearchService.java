...
public class McpBrowserSearchService {
    ...
    public List<SearchResult> search(String query, int maxResults) {
        ...
        // 使用默认值如果未指定
        if (maxResults <= 0) {
            maxResults = mcpConfig.getBrowserSearch().getDefaultMaxResults();
        }
        ...
        try {
            ...
            String optimizedQuery = mcpConfig.getBrowserSearch().isEnableQueryOptimization() 
    ...
    private List<SearchResult> searchWithSearXNG(String query, int maxResults) {
        try {
            ...
            // 使用配置的默认搜索引擎
            String enginesToUse = mcpConfig.getBrowserSearch().getDefaultEngines();
