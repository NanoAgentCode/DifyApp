...
| 能力 | 开源组件 | 用途 |
| --- | --- | --- |
| 邮件发送 | Spring Boot Mail / Jakarta Mail | 通过 SMTP 发送响应式 HTML 验证码邮件，并提供纯文本兜底内容 |
...
MAIL_SENDER_NAME=DifyApp
EMAIL_BRAND_NAME=DifyApp
EMAIL_BRAND_COLOR=#409EFF
MAIL_SMTP_AUTH=true
...
| `MAIL_SENDER_NAME` | 邮箱客户端中显示的发件人名称，默认 `DifyApp` |
| `EMAIL_BRAND_NAME` | 邮件标题、页头和正文使用的品牌名称，默认 `DifyApp` |
| `EMAIL_BRAND_COLOR` | 邮件强调色，必须是 `#RRGGBB` 格式，默认 `#409EFF` |
