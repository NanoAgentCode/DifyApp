...
- 已按当前实现全量校准。
- 已对齐按 key/分组读取配置接口与可观测性/日志配置联动说明。
- 2026-05 已补充页面助手对问答模型配置的复用方式。
...
用户端全局页面助手不新增独立模型配置，默认复用智能问答模型选择规则。前端请求可以传入 `modelId`；未传入时，后端沿用 `ChatService` 的问答模型解析逻辑，优先使用默认问答模型或可用于 `chat/both` 场景的启用模型。
...
- 页面助手模型复用：页面助手通过 `AssistantService` 适配为 `ChatRequest`，复用 `QA_MODEL` 中 `use_for=chat` 或 `use_for=both` 的问答模型
...
**页面助手使用说明：**
- 页面助手属于通用问答增强能力，使用 `QA_MODEL.use_for=chat` 或 `both` 的模型。
- 页面助手不使用向量化模型和向量数据库配置，不产生文档向量化成本。
- 页面助手可通过请求中的 `modelId` 指定模型；未指定时使用系统默认问答模型。
- 页面助手默认关闭浏览器检索和备忘录创建，这两个开关由 `AssistantServiceImpl` 转换 `ChatRequest` 时设置，不依赖系统配置项。
...
| type | VARCHAR(20) | 数据库类型（qdrant, milvus, faiss, chroma, weaviate, elasticsearch, pgvector） | NOT NULL |
...
**页面助手相关说明：** 页面助手复用该模型列表中的问答模型。若希望页面助手可用，应至少启用一个 `use_for=chat` 或 `use_for=both` 的问答模型，并设置默认模型或在前端请求中指定 `modelId`。
...
**路径参数：**
- `type`：数据库类型（qdrant, milvus, faiss, chroma, weaviate, elasticsearch, pgvector）
...
**接口路径：** `GET /api/data-sources/{id}`

**响应格式：** 数据源详情（同创建接口响应结构）
...
**接口路径：** `DELETE /api/data-sources/{id}`
...
**A**: 支持 Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch、PgVector。
