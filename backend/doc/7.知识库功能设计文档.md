...
- 已按当前实现全量校准。
- 已对齐知识库问答链路中的追踪接入、步骤采集、会话 ID 统一解析与上下文压缩相关优化。
- 2026-05 已补充知识库问答与用户端全局页面助手的边界说明。

## 1. 概述

### 1.1 功能简介

知识库功能是 DifyApp 系统的核心模块之一，提供了完整的知识库管理、文档管理、智能文档分块、文档向量化和基于 RAG（检索增强生成）的智能问答能力。该功能支持多种文档格式，集成了多种向量数据库，实现了从文档上传到智能问答的完整流程。系统采用智能分块策略系统，根据文件类型和内容特征自动选择合适的分块方式，无需用户配置。此外，系统还集成了 OCR（光学字符识别）服务，支持对图片文件和PDF文件进行文字识别，以及对Word文档中的图片和表格进行识别，从而实现对非文本文档的完整处理。

用户端全局页面助手不复用知识库索引，也不会为每个页面动态创建知识库。页面助手面向当前页面可见内容的即时问答，采用页面上下文 JSON 动态传递；知识库问答面向稳定文档资料，采用文档解析、分块、向量化、召回和答案生成流程。两者在使用场景和实现成本上保持分离。

### 1.2 功能目标

- 提供知识库的完整生命周期管理（创建、编辑、删除、查询）
- 支持多种文档格式的上传、管理和下载
- 实现文档的自动解析、智能分块和向量化处理
- 支持根据文件类型和内容特征自动选择分块策略
- 提供基于向量检索的智能问答功能
- 支持混合检索（全文检索 + 向量检索），提升检索准确率
- 支持流式和非流式两种问答模式
- 实现细粒度的权限控制和多租户隔离
- 支持多种向量数据库（Qdrant、FAISS、Milvus、Chroma、Weaviate、PgVector、Elasticsearch）
- 集成 OCR 服务，支持图片文件和PDF文件的文字识别
- 支持 Word 文档中图片和表格的 OCR 识别，实现完整文档内容提取
- 智能分块策略系统，根据文件类型和内容特征自动选择分块方式
...
- 文档重新向量化
- 图片文件和PDF文件的OCR识别
- Word文档中图片和表格的OCR识别和内容合并
- 智能分块策略选择（根据文件类型和内容特征自动选择）
...
页面助手和知识库问答都能回答用户问题，但上下文来源不同：

| 能力 | 上下文来源 | 关键实现 | 适用场景 |
| --- | --- | --- | --- |
| 知识库问答 | 已上传并向量化的文档块 | 文档解析、智能分块、Embedding、向量召回、LLM 生成 | 稳定知识、制度文档、产品资料、长期复用资料 |
| 页面助手 | 当前页面可见内容、选中文本、页面标题和路由 | 前端采集页面上下文 JSON、后端清洗裁剪、复用 `ChatService` | 当前页面解释、表格总结、字段说明、配置检查 |

**关键实现方式：**
- 页面助手不调用 `KnowledgeBaseQAService`，不访问向量库，也不生成知识库文档块。
- 页面助手不传入 `knowledgeBaseId`，会话历史使用 `type=5`；知识库问答历史使用 `type=2`。
- 如果页面展示的是知识库列表或文档列表，页面助手只基于页面已展示内容回答，不越权读取未展示文档全文。
- 如果用户需要基于完整文档内容问答，应进入知识库问答页面，由 `KnowledgeBaseQAService` 走 RAG 链路。
...
**查询参数：**
- `force`：可选，是否强制创建（名称重复时是否覆盖），默认 false
...
**接口路径：** `POST /api/knowledge-bases/{id}/generate-summary`

**查询参数：**
- `modelId`：可选，用于生成的问答模型 ID，不传则使用默认模型
...
**接口路径：** `GET /api/knowledge-bases/{id}/export`

**响应格式：** 文件流（ZIP 包，包含知识库元数据与文档）

#### 4.1.8 预览导入 ZIP

**接口路径：** `POST /api/knowledge-bases/import/preview`
...
**响应格式：** 预览结果（知识库信息、文档列表等）

#### 4.1.9 导入知识库

**接口路径：** `POST /api/knowledge-bases/import`

**请求方式：** `multipart/form-data`

**请求参数：**
- `file`：ZIP 文件（必填）
- `knowledgeBaseName`：知识库名称（必填）
- `description`：描述（可选）
- `vectorStoreType`：向量存储类型（可选）
- `isPublic`：是否公开（可选）
- `topK`：Top-K 检索数量（可选）
- `embeddingModelId`：向量化模型 ID（可选）

**响应格式：** 导入结果（知识库 ID、文档数量等）
...
- 知识库问答接口必须指定 `kbId`，服务端会校验用户是否可见该知识库，并从向量库召回相关文档片段。
- 页面助手接口为 `/api/assistant` 和 `/api/assistant/stream`，请求体携带 `pageContext`，不指定知识库 ID，不执行向量召回。
...
flowchart TD
    ...
    D -->|向量搜索| E[向量数据库检索]
    D -->|混合搜索| F[全文检索 + 向量检索]
    E --> G[构建上下文<br/>历史对话+检索]
    F --> G
    G --> H[上下文压缩<br/>可选]
    H --> I[生成答案<br/>LLM API]
    I --> J[保存对话历史<br/>PostgreSQL]
    J --> K[返回答案<br/>流式/非流式]
...
- **混合检索**：结合全文检索（BM25/关键词）与向量检索，并在结果端进行分数归一化（RRF 或权重加权）
- **知识库级别配置**：每个知识库可配置独立的 Top-K 值和检索模式
...
系统根据文件类型和内容特征自动选择合适的分块方式，无需用户配置。

**支持的分块策略：**

1. **固定大小分块（FixedSizeChunkStrategy）**
   - 按固定字符数分块，优化边界检测
   - 避免截断单词和句子
   - 适用于未知类型或无法识别的文件

2. **段落分块（ParagraphChunkStrategy）**
   - 按段落边界分块（双换行、段落标记）
   - 支持段落合并和拆分
   - 适用于纯文本文件（.txt）、Word文档、PDF文档

3. **句子分块（SentenceChunkStrategy）**
   - 按句子边界分块（中英文标点）
   - 支持句子合并以接近目标大小
   - 适用于需要精细语义的场景

4. **标题分块（HeadingChunkStrategy）**
   - 按标题层级组织分块
   - 识别 Markdown 标题（#, ##, ###）
   - 适用于 Markdown 文件

5. **代码分块（CodeChunkStrategy）**
   - 按代码块边界分块
   - 识别 Markdown 代码块和代码文件
   - 保持代码完整性
   - 适用于代码文件（.java, .js, .py等）

6. **表格分块（TableChunkStrategy）**
   - 按表格行分块，保持表格行的完整性
   - 支持 Markdown 表格、CSV 表格
   - 保留表头信息
   - 适用于表格文件（.xls, .xlsx, .csv）

7. **递归分块（RecursiveChunkStrategy）**
...
| 文件类型 | 检测内容 | 选择策略 |
|---------|---------|---------|
| .java, .js, .py等 | 代码文件 | 代码分块 |
| .xls, .xlsx, .csv | 表格文件 | 表格分块 |
| .md | 检测到标题 | 标题分块 |
| .md | 检测到表格+文本 | 混合策略（标题+表格+段落） |
| .doc, .docx | 检测到表格 | 混合策略（表格+段落） |
| .doc, .docx | 无表格 | 段落分块 |
| .pdf | 检测到表格 | 混合策略（表格+段落） |
| .pdf | 无表格 | 段落分块 |
| .txt | - | 段落分块 |
| 其他/未知 | - | 固定大小分块（默认） |
...
对于包含多种内容类型的文档（如 Word 文档中的表格+文本），系统会：
1. 使用内容分析器识别文档结构（表格、代码块、标题等）
2. 为每种内容类型选择对应的分块策略
3. 分别处理并合并结果，保持原始顺序
...
if (strategies.size() > 1) {
    ...
    List<ChunkResult> chunkResults = mixedContentChunker.chunk(structure, config);
} else {
    // 使用单个策略
    ChunkStrategy strategy = strategies.get(0);
    List<ChunkResult> chunkResults = strategy.chunk(text, config);
}
```

**表格识别增强：**

- **Word 文档**：使用 Apache POI 提取表格结构，转换为 Markdown 格式
- **Markdown 文档**：使用正则表达式识别表格结构
- 在解析结果中保留表格标记，供分块器识别

**前端提示：**

文档上传成功后，系统会自动检测分块策略并在前端显示提示信息，例如：
- "智能分块提醒：代码分块"
- "智能分块提醒：段落分块（自动检测表格）"
...
- **PgVector**：PostgreSQL 扩展，支持向量存储和检索（文档解读模块默认）
...
**实现原理：**
混合检索结合了关键词匹配（如 BM25）和向量语义搜索。系统会自动根据向量数据库的特性选择最佳实现方式：

- **Elasticsearch**：使用 `script_score` 或 `bool` 查询结合文本匹配得分与 `cosineSimilarity` 向量得分。
- **Weaviate**：使用 GraphQL 的 `hybrid` 运算符，支持动态调节 `alpha` 参数（0 为纯关键词，1 为纯向量）。

**支持状态：**
目前 **Elasticsearch** 和 **Weaviate** 已支持混合检索方案。对于不支持的数据库，系统将自动退化为纯向量检索。

**分数归一化：**
由于不同检索方式的原始分值范围不一，系统会对混合检索结果进行归一化处理，确保返回的 `score` 落在 0-1 之间，便于排序和阈值过滤。
...
rag:
  retrieval:
    ...
    hybrid:
      enabled: true            # 是否启用混合检索（需向量数据库支持）
      alpha: 0.5               # 权重参数（Weaviate 支持）
```
