...
用户端全局页面助手可以解释当前 AI 绘图页面上已经展示的图表、历史记录或配置，但不调用 `DrawIOService` 生成、修改或保存图表。需要真正生成或修改图表时，仍应使用 AI 绘图模块的专用接口。
...
| 能力 | 目标 | 关键实现 | 是否写图表数据 |
| --- | --- | --- | --- |
| AI 绘图 | 生成/修改 Mermaid 图表 | `DrawIOService` 调用 LLM 生成 Mermaid 并持久化图表/历史 | 是 |
| 页面助手 | 解释当前页面可见内容 | `GlobalAssistant` 采集页面上下文，`AssistantService` 复用 `ChatService` 问答 | 否 |

**关键实现方式：**
- 页面助手提示词明确限制为问答和建议，不声称已执行保存、修改、删除等操作。
- 页面助手只读取当前页面文本和结构化上下文，不调用 `/api/drawio/generate`、`/api/drawio/modify`、`/api/drawio/save`。
- 如果绘图页面注册页面级 `useAssistantContext`，应输出图表标题、Mermaid 摘要、当前历史项等业务 JSON，而不是完整 DOM 或大段 SVG。
