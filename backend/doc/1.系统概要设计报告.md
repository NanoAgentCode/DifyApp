...
- 本文档已按当前实现进行全量同步校准。
- 已纳入可观测性升级（`ops.trace`、步骤追踪、异步一致性 `PARTIAL`、前端日志监控增强）。
- 已纳入链路一致性规则（同请求多记录共享 `traceId`，`spanId` 唯一）。
- 已纳入追踪双机制策略（业务主追踪优先，AOP 追踪降级兜底，门控去重）。
- 2026-05 已补充用户端全局页面助手的系统定位、接口、历史类型和设计文档索引。

## 1. 项目概述

### 1.1 项目背景
DifyApp 是一个基于 Spring Boot 和 Vue 3 的综合性 AI 应用管理平台，旨在提供完整的 AI 应用管理、知识库管理、智能问答、用户端全局页面助手、AI 绘图等功能。系统集成了 Dify AI 平台，支持多租户架构，提供细粒度的权限控制，为企业和个人用户提供统一的 AI 应用管理平台。系统采用 RAG（检索增强生成）技术实现知识库问答，支持混合检索（全文检索 + 向量检索）以提升准确率，支持多种向量数据库和 LLM 模型，提供流式和非流式两种响应模式，具备完整的对话历史管理、内存管理以及**备忘录意图识别与确认创建**能力。用户端全局页面助手通过采集当前页面的结构化上下文 JSON，实现任意页面内的轻量问答，进一步提升用户交互效率。

### 1.2 项目目标
- 提供完整的 AI 应用生命周期管理（创建、编辑、删除、查看）
- 支持 Chat Flow 和 Workflow 两种应用类型的调用
- 实现知识库的创建、管理和文档向量化
- 提供基于 RAG（检索增强生成）的智能问答功能，支持混合检索提升准确度
- 提供通用智能问答功能（不使用知识库）
- 提供用户端全局页面助手，支持基于当前页面内容的即时问答
- 支持流式和非流式响应模式
- 实现完整的对话历史管理功能
- 提供 AI 绘图功能（基于 Mermaid）
- 提供系统配置管理功能（模型、向量数据库、数据源、系统配置）
- 实现**备忘录意图识别与用户确认创建**，防止模型误称已执行写入操作
- 提供多租户和权限管理能力
...
- **主要组件**：
  ...
  - `AssistantController`：用户端全局页面助手问答
  ...
  - `AdminDataAnalysisController`：数据分析管理（管理员端）
  - `AdminUserActionLogController`：用户行为日志管理（管理员端）
  - `UserMemoryController`：用户记忆管理（用户端）
  - `AdminUserMemoryController`：用户记忆管理（管理员端）
...
- **主要组件**：
  ...
  - `AssistantService`：页面助手请求适配、上下文清洗和提示词构造服务
  ...
  - `DataAnalysisService`：数据分析服务（Neo4j 图数据库同步）
  - `UserActionLogService`：用户行为日志服务（Elasticsearch 日志存储）
  - `ElasticsearchLogService`：Elasticsearch 日志服务
  - `ContextCompressionService`：上下文压缩服务
  - `UserMemoryService`：用户记忆管理服务（长期记忆和实体记忆）
...
- **主要组件**：
  ...
  - `UserActionLogRepository`：用户行为日志数据访问（关系数据库）
...
- **上下文压缩**：优化长对话性能
- **自动保存历史**：自动保存对话记录
- **备忘录意图识别**：调用模型前解析提醒需求，生成候选项，用户确认后创建备忘录
...
#### 4.3.4.1 功能概述
用户端全局页面助手是在用户端任意页面可打开的轻量问答入口。它基于当前页面可见内容、选中文本、页面标题和路由等上下文回答问题，不为每个页面单独构建知识库，而是将页面核心内容组织为结构化 JSON 动态传递给大模型。

#### 4.3.4.2 核心功能
- **全局悬浮入口**：在用户端布局中提供右侧悬浮按钮和右侧抽屉窗口。
- **页面上下文采集**：前端采集当前页面标题、路由、选中文本和主要内容区块。
- **结构化上下文**：优先使用页面级 provider 输出业务 JSON；未注册时回退到 DOM 可见文本采集。
- **HTML 噪声过滤**：过滤布局、菜单、弹窗、脚本、样式、助手自身等无效内容。
- **服务端二次清洗**：后端剥离 HTML 标签、脱敏敏感字段、限制区块和历史长度。
- **复用通用问答链路**：通过 `AssistantService` 适配为 `ChatRequest`，复用 `ChatService` 的模型调用、流式响应和历史保存能力。
- **历史标记**：页面助手问答写入对话历史，使用 `chat_conversation.type=5` 标记。

#### 4.3.4.3 技术实现
- 前端组件：`GlobalAssistant.vue` 挂载在 `UserLayout.vue`。
- 上下文采集：`assistantContextCollector.js` 和 `useAssistantContext.js`。
- 后端接口：`AssistantController` 提供 `/api/assistant` 和 `/api/assistant/stream`。
- 上下文处理：`AssistantContextSanitizer` 负责白名单清洗、HTML 剥离、脱敏与裁剪。
- 提示词构造：`AssistantPromptBuilder` 将页面上下文 JSON 和用户问题组合为页面助手提示词。
- 运行边界：页面助手只做问答和建议，不直接执行创建、修改、删除、提交等系统动作。

**详细设计文档**：`18.用户端全局页面助手设计文档.md`
...
- **非流式问答**：一次性返回完整答案
- **流式问答**：实时返回答案片段（SSE）
- **混合检索**：结合全文检索（关键词/BM25）与向量相似度检索（支持 Elasticsearch/Weaviate）
...
### 4.6 GraphRAG 知识图谱问答模块

#### 4.6.1 功能概述
基于 Neo4j 图数据库提供管理员侧知识图谱问答能力。系统将用户、应用、知识库、文档、会话、消息和问答模型等实体同步为图谱，并支持基于图谱的可视化分析、快速问答和 GraphRAG 知识问答。

#### 4.6.2 核心功能
- **图谱同步**：将核心业务实体和关系同步到 Neo4j。
- **图谱展示**：支持关键词、节点类型、关系类型和 1-3 跳关系筛选。
- **快速问答**：基于规则意图和 Cypher 模板回答统计、列表和基础关系问题。
- **知识问答**：支持实体识别、多跳召回、召回排序、LLM 生成和引用编号。
- **引用核验**：服务端校验答案中的 `[G1]`、`[G2]` 等引用是否来自真实召回结果，前端支持点击引用定位证据。
- **错误降级**：无上下文、模型不可用、模型生成失败或引用异常时，降级为结构化图谱依据。
- **可观测性**：接入行为日志、业务 Trace、步骤 Trace、慢查询日志和响应指标。

#### 4.6.3 技术实现
- Neo4j Bolt 驱动访问图数据库。
- 图谱标签和关系类型白名单，避免任意 Cypher 参数注入。
- LangChain4j 问答模型调用，用于 GraphRAG 答案生成。
- Prompt 来源数量、问题长度、召回数量和关系深度限制，用于控制安全和成本。
- 前端使用 Vue 3 和 ECharts 展示图谱，节点详情采用弹窗展示，问答区域展示引用、指标和降级原因。

**详细设计文档**：`17.GraphRAG知识图谱问答设计文档.md`
...
#### 4.7.3 技术实现
- 会话和消息分离存储
- 支持会话类型区分（普通聊天、知识库问答、文档问答、Agent 任务、页面助手）
- 分页查询优化
- 软删除机制
- 自动生成会话标题
...
#### 4.10.1 功能概述
提供用户长期记忆和实体记忆的管理功能，用于在多轮对话中沉淀"长期可复用"的用户信息，并在后续提问时作为上下文注入到系统提示词中，提升回答的个性化、一致性与任务连续性。
...
- type: INTEGER (类型：1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务，5-页面助手)
- title: VARCHAR (会话标题)
- model_id: BIGINT (会话使用的模型ID)
- summary: TEXT (会话滚动摘要)
- summary_updated_sequence: INTEGER (摘要已覆盖到的消息序号)
- summary_update_time: TIMESTAMP (摘要更新时间)
- created_time: TIMESTAMP
- updated_time: TIMESTAMP
```
...
#### 5.2.18 用户记忆表 (USER_MEMORY)
```sql
- id: BIGSERIAL (主键)
- user_id: BIGINT (用户ID)
- scope_type: VARCHAR(32) (作用域类型：chat/knowledge_base/app)
- scope_id: BIGINT (作用域ID，知识库/应用ID，chat为空)
- memory_type: VARCHAR(32) (记忆类型：long_term/entity)
- memory_key: VARCHAR(200) (记忆键，用于去重更新)
- content: TEXT (记忆内容)
- importance: INTEGER (重要度，0-5)
- first_seen_time: TIMESTAMP (首次记录时间)
- last_mentioned_time: TIMESTAMP (最近在对话中提及或更新的时间)
- last_accessed_time: TIMESTAMP (最近被检索并注入上下文的时间)
- access_count: INTEGER (被检索使用次数)
- source_conversation_id: BIGINT (最近来源会话ID)
- create_time: TIMESTAMP (创建时间)
- update_time: TIMESTAMP (更新时间)
- deleted: INTEGER (是否删除：0-未删除，1-已删除)
```

唯一约束：`UNIQUE (user_id, scope_type, scope_id, memory_type, memory_key)`
```sql
- id: BIGINT (主键)
- user_id: BIGINT (用户ID，外键)
- data_source_id: BIGINT (数据源ID，外键)
...
- 用户记忆表：`user_id`、`scope_type`、`scope_id`、`memory_type`、`deleted` 索引，`(user_id, scope_type, scope_id, memory_type)` 复合索引
...
- `POST /api/assistant` - 用户端全局页面助手问答（非流式）
- `POST /api/assistant/stream` - 用户端全局页面助手问答（流式）
...
#### 6.3.12 记忆管理 API
- `GET /api/memory/items` - 查看我的记忆（用户端）
- `DELETE /api/memory/clear` - 清空我的记忆（用户端）
- `DELETE /api/memory/items/{id}` - 删除我的记忆条目（用户端）
- `GET /api/admin/memory/users/{userId}/items` - 查看用户记忆（管理员端）
- `DELETE /api/admin/memory/users/{userId}/clear` - 清空用户记忆（管理员端）
...
8. **用户行为日志功能设计文档.md** - 用户行为日志采集、Elasticsearch 存储、日志查询和分析
9. **记忆模块功能设计文档.md** - 用户长期记忆和实体记忆管理、记忆抽取和注入
10. **用户端全局页面助手设计文档.md** - 页面助手上下文采集、JSON 化、清洗裁剪、问答链路复用和历史标记
