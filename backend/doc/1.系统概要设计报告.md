...
### 1.1 项目背景
DifyApp 是一个基于 Spring Boot 和 Vue 3 的综合性 AI 应用管理平台，旨在提供完整的 AI 应用管理、知识库管理、智能问答、Text2SQL、AI 绘图等功能。系统集成了 Dify AI 平台，支持多租户架构，提供细粒度的权限控制，为企业和个人用户提供统一的 AI 应用管理平台。系统采用 RAG（检索增强生成）技术实现知识库问答，支持多种向量数据库和 LLM 模型，提供流式和非流式两种响应模式，具备完整的对话历史管理和系统配置管理能力。
...
flowchart TB
    ...
    subgraph Storage["数据存储层"]
        direction LR
        C1[PostgreSQL<br/>关系数据库] --- C2[MinIO<br/>对象存储] --- C3[向量数据库<br/>Qdrant/Milvus等]
    end
...
- **主要组件**：
  ...
  - `Text2SqlController`：Text2SQL 功能
...
- **主要组件**：
  ...
  - `TableSchemaCacheRepository`：表结构缓存数据访问
...
| BCrypt | - | 密码加密 |
| MinIO | 8.5.2+ | 对象存储 |
| Qdrant/Milvus/FAISS/Chroma/Weaviate/Elasticsearch | - | 向量数据库（多选一） |
...
- **关系型数据库**：MySQL、PostgreSQL、Oracle、MongoDB（Text2SQL）
...
- Apache Tika 文档解析
- LangChain4j 文档分块
- 多种向量数据库支持（Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch）
- 异步处理文档向量化
...
#### 4.7.1 功能概述
提供自然语言转 SQL 的功能，支持多种关系型数据库。

#### 4.7.2 核心功能
- **自然语言查询**：将自然语言问题转换为 SQL 查询
- **多数据库支持**：MySQL、PostgreSQL、Oracle、MongoDB
- **表结构管理**：自动获取和缓存数据库表结构
- **SQL 执行**：执行生成的 SQL 并返回结果
- **安全控制**：SQL 注入防护、只读模式
...
#### 5.2.13 表结构缓存表 (TABLE_SCHEMA_CACHE)
```sql
- id: BIGINT (主键)
...
- 表结构缓存表：`data_source_id`、`table_name` 索引
...
- **PostgreSQL 15+**：关系数据库
- **MinIO**：对象存储服务
- **向量数据库**：Qdrant、Milvus、FAISS、Chroma、Weaviate 或 Elasticsearch（至少一种）
- **Java 运行环境**：JDK 1.8+

#### 8.1.2 可选服务
- **Nginx**：反向代理和负载均衡
- **Redis**：缓存服务（未来扩展）
- **关系型数据库**：MySQL、PostgreSQL、Oracle、MongoDB（Text2SQL 功能需要）
...
- 可配置的文档分块和检索参数
- 支持上下文压缩，优化长对话性能
- 支持多种向量数据库（Qdrant、Milvus、FAISS、Chroma、Weaviate、Elasticsearch）
...
6. **Text2SQL功能设计文档.md** - 自然语言转 SQL、多数据库支持
7. **对话历史管理功能设计文档.md** - 会话管理、消息管理、统计分析
8. **系统配置管理功能设计文档.md** - 模型管理、向量数据库管理、数据源管理、系统配置
