...
- 已按当前实现全量校准。
- 已对齐应用调用链路中的追踪治理与系统配置读取规范。
- 2026-05 已补充 AI 应用调用与页面助手的边界说明。
...
用户端全局页面助手不调用 Dify 应用，也不读取 `AI_APP` 配置。页面助手属于 DifyApp 本地通用问答链路的页面上下文适配能力；AI 应用管理模块面向外部 Dify Chat Flow/Workflow 应用的配置和调用，两者在模型来源、接口路径、权限模型和会话类型上保持分离。
...
| 能力 | 后端入口 | 外部依赖 | 会话类型 | 关键实现 |
| --- | --- | --- | --- | --- |
| AI 应用 Chat Flow/Workflow | `/api/ai-apps/{id}/...` | Dify API | 应用会话 | `AiAppService` + `DifyApiClient` |
| 页面助手 | `/api/assistant`、`/api/assistant/stream` | 本地问答模型 | `type=5` | `AssistantService` + `ChatService` |

**关键实现方式：**
- 页面助手不使用 `appId`，不会读取 Dify API Key，也不会调用 Dify Chat Flow/Workflow。
- AI 应用调用依赖用户对应用的可见性权限；页面助手依赖当前用户已打开页面的可见内容。
- 若页面展示 AI 应用列表，页面助手只能解释当前页面已展示的应用信息，不会越权读取不可见应用配置。
...
**页面助手说明：** 页面助手当前不复用 AI 应用文件上传能力，也不向 Dify 上传文件。页面助手上下文来自当前页面文本、选中文本和页面级结构化 provider。
