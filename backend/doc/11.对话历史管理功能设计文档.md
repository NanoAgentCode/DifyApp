...
- 2026-05 已补充页面助手会话类型（type=5）说明。

## 1. 概述

### 1.1 功能简介

对话历史管理功能是 DifyApp 系统的核心模块之一，负责管理用户与 AI 系统的所有对话记录。该功能采用会话（Conversation）和消息（Message）的两级结构，支持普通聊天、知识库问答、文档问答、Agent 任务模式和页面助手等类型的对话历史管理。系统提供了用户端和管理员端两套接口，用户端可以管理自己的对话历史，管理员端可以查看所有用户的对话历史并进行统计分析。功能包括会话的创建、查询、更新、删除、导出，以及消息的保存和查询等完整能力。

### 1.2 功能目标

- 提供完整的对话历史管理能力
- 支持会话和消息的两级数据结构
- 支持智能问答、知识库问答、文档问答、Agent 任务和页面助手等类型
- 提供用户端和管理员端两套接口
- 支持会话标题的自动生成和手动更新
- 支持会话的软删除机制
- 支持会话导出功能
...
| type | INTEGER | 会话类型（1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务，5-页面助手） | DEFAULT 1 |
| title | VARCHAR(500) | 会话标题 | |
| model_id | BIGINT | 会话使用的模型ID | |
| summary | TEXT | 会话滚动摘要，用于长会话上下文压缩 | |
...
- INDEX idx_chat_conversation_summary_update_time (summary_update_time)
...
- `type=4`：Agent 任务模式
- `type=5`：页面助手
- 当会话超过配置轮次后，系统会异步生成 `summary`；之后每新增一定轮次滚动更新一次，摘要以 `summary_updated_sequence` 标记覆盖范围。
...
### 3.3 Agent 任务运行表 (agent_task_run)

任务模式使用任务运行表保存一次 Agent 任务的主状态。任务会话仍属于 `chat_conversation`，并通过 `conversation_id` 关联。

| 字段名 | 类型 | 说明 | 约束 |
|--------|------|------|------|
| id | BIGINT | 主键 | PRIMARY KEY |
| run_id | VARCHAR(64) | 任务运行唯一标识 | UNIQUE, NOT NULL |
| conversation_id | BIGINT | 会话ID | NOT NULL |
| user_id | BIGINT | 用户ID | NOT NULL |
| question | TEXT | 用户原始任务 | |
| status | VARCHAR(40) | 任务状态 | |
| model_id | BIGINT | 模型ID | |
| enable_browser_search | BOOLEAN | 是否启用浏览器检索 | |
| is_admin | BOOLEAN | 发起用户是否管理员 | |
| final_answer | TEXT | 最终答案 | |
| pending_confirmation_id | VARCHAR(64) | 待确认ID | |
| create_time/update_time/deleted | 通用字段 | 审计与软删除 | |

**状态说明：**
- `PENDING`：任务已创建，尚未开始执行。
- `RUNNING`：任务执行中。
- `WAITING_CONFIRMATION`：等待用户确认高风险动作。
- `CONFIRMED`：用户已确认，等待继续执行。
- `SUCCEEDED`：任务成功完成。
- `FAILED`：任务失败。
- `CANCELLED`：用户拒绝确认或取消任务。

### 3.4 Agent 任务步骤表 (agent_task_step)

任务步骤表保存可恢复、可审计的结构化执行事件。前端打开任务会话时，会按这些事件恢复任务执行时间线。

| 字段名 | 类型 | 说明 | 约束 |
|--------|------|------|------|
| id | BIGINT | 主键 | PRIMARY KEY |
| run_id | VARCHAR(64) | 任务运行唯一标识 | NOT NULL |
| task_id | BIGINT | 任务运行表主键 | NOT NULL |
| conversation_id | BIGINT | 会话ID | NOT NULL |
| step_index/step_number | INTEGER | 步骤序号 | |
| event_type | VARCHAR(60) | 事件类型 | |
| status | VARCHAR(40) | 事件状态 | |
| content | TEXT | 展示内容 | |
| tool_name | VARCHAR(120) | 工具名称 | |
| tool_input_summary | TEXT | 工具输入摘要 | |
| tool_output_summary | TEXT | 工具输出摘要 | |
| requires_confirmation | BOOLEAN | 是否需要确认 | |
| confirmation_id | VARCHAR(64) | 确认ID | |
| risk_level | VARCHAR(40) | 风险级别 | |
| error | TEXT | 错误信息 | |
| create_time/update_time/deleted | 通用字段 | 审计与软删除 | |
...
**保存原则：**
- `chat_message` 仅保存用户原始请求和最终助手回复。
- 任务执行过程保存到 `agent_task_step`。
- 历史列表默认混合展示，不按任务/问答拆分；任务会话可在前端显示“任务”标签。
...
- `type`：会话类型（1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务，5-页面助手，可选，默认1）
- `appId`：应用ID（可选）
...
- `type`：会话类型（可选，1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务，5-页面助手）
...
Agent 任务模式复用会话历史入口，但保存内容拆分为两类：

1. 创建任务时，先创建或复用 `type=4` 的 `chat_conversation`。
2. 任务执行过程中的计划、步骤、工具调用和观察结果写入 `agent_task_step`。
3. 任务完成后，用户原始任务写入 `chat_message(role=user)`。
4. 最终答案写入 `chat_message(role=assistant)`。
5. 打开任务会话详情时，先加载 `chat_message`，再加载任务步骤并组装为前端可展示的任务消息。
