...
对话历史管理功能是 DifyApp 系统的核心模块之一，负责管理用户与 AI 系统的所有对话记录。该功能采用会话（Conversation）和消息（Message）的两级结构，支持普通聊天、知识库问答、文档问答和 Agent 任务模式等类型的对话历史管理。系统提供了用户端和管理员端两套接口，用户端可以管理自己的对话历史，管理员端可以查看所有用户的对话历史并进行统计分析。功能包括会话的创建、查询、更新、删除、导出，以及消息的保存和查询等完整能力。
...
| type | INTEGER | 会话类型（1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务） | DEFAULT 1 |
...
- `type`：会话类型（1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务，可选，默认1）
- `appId`：应用ID（可选）
...
- `type`：会话类型（可选，1-普通聊天，2-知识库问答，3-文档问答，4-Agent任务）
