...
知识库功能是 DifyApp 系统的核心模块之一，提供了完整的知识库管理、文档管理、文档向量化和基于 RAG（检索增强生成）的智能问答能力。该功能支持多种文档格式，集成了多种向量数据库，实现了从文档上传到智能问答的完整流程。
...
- 支持流式和非流式两种问答模式
- 实现细粒度的权限控制和多租户隔离
- 支持多种向量数据库（Qdrant、FAISS、Milvus、Chroma、Weaviate、Elasticsearch）
...
flowchart TB
    ...
    subgraph Service["业务逻辑层"]
        direction LR
        C1[KnowledgeBase<br/>Service] --- C2[Document<br/>Service] --- C3[DocumentVectorization<br/>Service] --- C4[RAG Retrieval<br/>Service] --- C5[Context Compression<br/>Service] --- C6[KnowledgeBaseQA<br/>Service]
    end
    
    subgraph Storage["数据存储层"]
        direction LR
        D1[PostgreSQL<br/>关系数据库] --- D2[MinIO<br/>对象存储] --- D3[向量数据库<br/>Qdrant等]
    end
...
flowchart TD
    ...
    D --> E[解析文档内容<br/>Apache Tika]
    E --> F[文档分块<br/>LangChain4j]
    F --> G[向量化文档片段<br/>Embedding API]
    G --> H[存储向量到数据库<br/>Qdrant等]
    H --> I[更新文档状态<br/>completed]
...
4. **文档解析**：使用 Apache Tika 解析文档内容，提取文本
5. **文档分块**：使用 LangChain4j 将文档分割成多个文本片段（chunks）
6. **向量化**：调用 Embedding API 将文本片段转换为向量
7. **向量存储**：将向量存储到向量数据库（Qdrant、FAISS 等）
8. **状态更新**：更新文档状态为 `completed`
